...
class IMUB_Head(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.down_times):
            # original usava F.upsample; trocado por interpolate (equivalente)
            inputs.append(F.interpolate(x, size=(x.shape[2] // (2 * i + 2), x.shape[3] // (2 * i + 2)), mode="bilinear", align_corners=False))
...
class UNet_Plus_Residual(nn.Module):
    def __init__(
        ...
        # FIX: não sobrescrever end_convs depois
        if self.multi_out:
...
# ----------------------------
# Optional helpers (não usados no pipeline, mas mantidos)
# ----------------------------
