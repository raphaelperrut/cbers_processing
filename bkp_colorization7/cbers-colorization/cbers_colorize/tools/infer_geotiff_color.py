...
def _log_vram(prefix: str = "[VRAM] ", reset_peak: bool = False) -> None:
    ...
    print(f"{prefix}alloc={alloc}MB reserved={rsv}MB peak_alloc={peak_alloc}MB peak_reserved={peak_rsv}MB")
...
def _multiscale_detail(
    ...
    b1 = gaussian_blur2d(pan01, sigma=sigma1, ksize=_kernel_size_from_sigma(sigma1))
    b2 = gaussian_blur2d(pan01, sigma=sigma2, ksize=_kernel_size_from_sigma(sigma2))
    b3 = gaussian_blur2d(pan01, sigma=sigma3, ksize=_kernel_size_from_sigma(sigma3))

    d1 = (pan01 - b1).astype(np.float32, copy=False)
...
def color_transfer_pan_rgbnir(
    ...
    if fp.mode not in ("multiscale_ycbcr", "ratio"):
    ...
    if fp.shadow_strength > 0 and fp.shadow_y_hi > fp.shadow_y_lo:
        t_bright = _smoothstep(float(fp.shadow_y_lo), float(fp.shadow_y_hi), y_g)
    ...
    pan_lp = gaussian_blur2d(pan, sigma=float(fp.pan_hp_sigma), ksize=_kernel_size_from_sigma(float(fp.pan_hp_sigma)))
    pan_hp = (pan - pan_lp).astype(np.float32, copy=False)
    ...
    y_fused = (
        y_g
        + detail_gain_map * y_detail
        + pan_gain_map * pan_hp_n
    ).astype(np.float32, copy=False)
...
def _process_window(
    *,
    ...
    if in_is_pan1:
        ...
        tile_in = np.stack([pan, pan, pan], axis=0).astype(np.float32, copy=False)
    else:
    ...
    *,
    ...
    with rasterio.open(src_path) as psrc, rasterio.open(out_path) as osrc:
        if psrc.width != osrc.width or psrc.height != osrc.height:
            print(
                f"[QA2] WARN: size mismatch PAN={psrc.width}x{psrc.height} OUT={osrc.width}x{osrc.height} (QA2 skipped)"
            )
            return
...
def main():
    ...
    ap.add_argument("--pkl", required=False)
    ...
    ap.add_argument("--guide_mode", default="pan_luminance_injection", choices=["none", "pan_luminance_injection"])
    ap.add_argument("--guide_only", action="store_true")

    ap.add_argument("--fusion_mode", default="multiscale_ycbcr", choices=["multiscale_ycbcr", "ratio"])
    ...
    if args.verbose and device == "cuda":
        print(
            ...
            f"scipy_fast={'yes' if _HAS_SCIPY else 'no'}"
        )
    ...
    ) as nirsrc, rasterio.open(out_path, "w", **prof_out) as dst:
        dst.colorinterp = (ColorInterp.red, ColorInterp.green, ColorInterp.blue)
