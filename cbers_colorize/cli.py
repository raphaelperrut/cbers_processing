...
def _validate_highlights(args) -> None:
    ...
    urban_hot_y = float(getattr(args, "urban_hot_y", 0.0))
    ...
    if not (0.0 <= urban_hot_y <= 1.0):
        raise ValueError("--urban_hot_y deve estar em 0..1")
    if not (0.0 <= urban_hot_strength <= 1.0):
        raise ValueError("--urban_hot_strength deve estar em 0..1")
    if not (0.0 <= final_softclip_margin <= 0.5):
        raise ValueError("--final_softclip_margin deve estar em 0..0.5")
...
def _validate_luma(args) -> None:
    ...
    if float(getattr(args, "veg_luma_from_guide", 0.0)) < 0.0 or float(getattr(args, "veg_luma_from_guide", 0.0)) > 1.0:
        raise ValueError("--veg_luma_from_guide deve estar em 0..1")
    if not (0.0 <= float(getattr(args, "simple_y_mix", 0.80)) <= 1.0):
        raise ValueError("--simple_y_mix deve estar em 0..1")
...
def _validate_guided(args) -> None:
    ...
    if int(args.residual_guided_radius) < 0:
        raise ValueError("--residual_guided_radius deve ser >= 0")
    if float(args.residual_guided_eps) <= 0.0:
        raise ValueError("--residual_guided_eps deve ser > 0")
    if not (0.0 <= float(args.residual_guided_mix) <= 1.0):
        raise ValueError("--residual_guided_mix deve estar em 0..1")
...
def _validate_radiometry(args) -> None:
    if str(args.radiometric_finish_mode).lower() not in ("none", "y_only", "rgb_global"):
        raise ValueError("--radiometric_finish_mode inválido")
...
def _validate_progress(args) -> None:
    if float(args.progress_throttle_s) < 0.0:
        raise ValueError("--progress_throttle_s deve ser >= 0")
...
def _validate_general(args) -> None:
    ...
    if str(args.guide_norm).lower() not in ("per_band", "joint_y"):
        raise ValueError("--guide_norm inválido")

    if str(args.pan_syn_mode).lower() not in ("rgbnir_global_robust",):
        raise ValueError("--pan_syn_mode inválido")


def _collect_explicit_options(argv: list[str]) -> set[str]:
    explicit = set()
    i = 0
    while i < len(argv):
        tok = argv[i]
...
def _set_if_not_explicit(args, explicit_opts: set[str], opt_name: str, value) -> None:
    if opt_name not in explicit_opts:
        setattr(args, opt_name[2:], value)


def _apply_profile(args, explicit_opts: set[str]) -> None:
    p = str(getattr(args, "profile", "balanced")).lower()

    if p == "balanced":
        _set_if_not_explicit(args, explicit_opts, "--guide_norm", "joint_y")
        if "--use_residual" not in explicit_opts and "--no_use_residual" not in explicit_opts:
            args.use_residual = True
        _set_if_not_explicit(args, explicit_opts, "--radiometric_finish_mode", "y_only")
        _set_if_not_explicit(args, explicit_opts, "--pan_syn_base_mix", 0.15)
        _set_if_not_explicit(args, explicit_opts, "--residual_guided_mix", 0.55)
        return
    ...
    if p == "spectral_safe":
        ...
        if "--use_residual" not in explicit_opts and "--no_use_residual" not in explicit_opts:
            args.use_residual = True
        _set_if_not_explicit(args, explicit_opts, "--radiometric_finish_mode", "none")
        _set_if_not_explicit(args, explicit_opts, "--chroma_strength", 0.66)
        _set_if_not_explicit(args, explicit_opts, "--sat", 0.88)
        _set_if_not_explicit(args, explicit_opts, "--guided_chroma_mix", 0.45)
        _set_if_not_explicit(args, explicit_opts, "--detail_strength_urban", 1.05)
        _set_if_not_explicit(args, explicit_opts, "--pan_hp_gain", 0.12)
        _set_if_not_explicit(args, explicit_opts, "--pan_syn_base_mix", 0.20)
        _set_if_not_explicit(args, explicit_opts, "--residual_guided_mix", 0.60)
        return
    ...
    if p == "urban_sharp":
        ...
        if "--use_residual" not in explicit_opts and "--no_use_residual" not in explicit_opts:
            args.use_residual = True
        _set_if_not_explicit(args, explicit_opts, "--radiometric_finish_mode", "y_only")
        _set_if_not_explicit(args, explicit_opts, "--detail_strength_urban", 1.30)
        _set_if_not_explicit(args, explicit_opts, "--detail_alpha1", 0.70)
        _set_if_not_explicit(args, explicit_opts, "--detail_alpha2", 0.24)
        _set_if_not_explicit(args, explicit_opts, "--detail_alpha3", 0.10)
        _set_if_not_explicit(args, explicit_opts, "--pan_hp_gain_urban", 1.55)
        _set_if_not_explicit(args, explicit_opts, "--pan_hp_gain", 0.18)
        _set_if_not_explicit(args, explicit_opts, "--guided_chroma_mix", 0.72)
        _set_if_not_explicit(args, explicit_opts, "--residual_guided_mix", 0.50)
        return

    if p == "vegetation_safe":
        _set_if_not_explicit(args, explicit_opts, "--guide_norm", "joint_y")
        if "--use_residual" not in explicit_opts and "--no_use_residual" not in explicit_opts:
            args.use_residual = True
        _set_if_not_explicit(args, explicit_opts, "--radiometric_finish_mode", "y_only")
        _set_if_not_explicit(args, explicit_opts, "--veg_sat", 0.78)
        _set_if_not_explicit(args, explicit_opts, "--veg_chroma", 0.88)
        _set_if_not_explicit(args, explicit_opts, "--detail_strength_veg", 0.45)
        _set_if_not_explicit(args, explicit_opts, "--pan_hp_gain_veg", 0.40)
        _set_if_not_explicit(args, explicit_opts, "--nir_detail_boost", 0.12)
        _set_if_not_explicit(args, explicit_opts, "--pan_syn_base_mix", 0.18)
        _set_if_not_explicit(args, explicit_opts, "--residual_guided_mix", 0.62)
        return

    raise ValueError(f"profile inválido: {args.profile}")


def main():
    explicit_opts = _collect_explicit_options(sys.argv[1:])
    ...
    run.add_argument(
        "--profile",
        default="balanced",
        choices=["balanced", "spectral_safe", "urban_sharp", "vegetation_safe"],
    )

    run.add_argument("--guide_norm", default="joint_y", choices=["per_band", "joint_y"])
    run.add_argument("--pan_syn_mode", default="rgbnir_global_robust", choices=["rgbnir_global_robust"])
    ...
    run.add_argument("--simple_y_mix", type=float, default=0.80)
    ...
    run.add_argument("--veg_luma_lift", type=float, default=0.0)
    run.add_argument("--veg_luma_from_guide", type=float, default=0.0)
    ...
    run.add_argument("--urban_hot_y", type=float, default=0.82)
    run.add_argument("--urban_hot_strength", type=float, default=0.0)
    run.add_argument("--final_softclip_margin", type=float, default=0.0)
    ...
    run.add_argument("--use_residual", action="store_true")
    run.add_argument("--no_use_residual", action="store_true")
    run.add_argument("--pan_syn_base_mix", type=float, default=0.15)
    run.add_argument("--residual_guided_radius", type=int, default=8)
    run.add_argument("--residual_guided_eps", type=float, default=1e-4)
    run.add_argument("--residual_guided_mix", type=float, default=0.55)

    run.add_argument("--radiometric_finish_mode", default="y_only", choices=["none", "y_only", "rgb_global"])
    ...
    run.add_argument(
        ...
        action="store_true",
        default=True,
        help="Habilita emissão de progresso no pipeline.",
    )
    run.add_argument(
        "--no_progress",
        dest="progress_enabled",
        action="store_false",
        help="Desabilita emissão de progresso no pipeline.",
    )
    run.add_argument(
        "--progress_throttle_s",
        type=float,
        default=0.2,
        help="Intervalo mínimo entre emissões de progresso, em segundos.",
    )
    ...
    run.add_argument("--export_ecw", dest="export_ecw", action="store_true", help="Gera pan_2m_color_guided.ecw como derivado opcional.")
    ...
    if args.cmd == "run":
        ...
        if getattr(args, "no_use_residual", False):
            args.use_residual = False
        elif getattr(args, "use_residual", False):
            args.use_residual = True
        else:
            args.use_residual = bool(getattr(args, "use_residual", True))

        _apply_profile(args, explicit_opts)

        if "--no_use_residual" in explicit_opts:
            args.use_residual = False
        elif "--use_residual" in explicit_opts:
            args.use_residual = True
        ...
        if args.verbose:
            print("[CLI] cmd=run")
            print("[CLI] profile:", args.profile)
            print("[CLI] explicit_opts:", sorted(explicit_opts))
            ...
            print("[CLI] simple_y_mix:", args.simple_y_mix)
            print("[CLI] guide_norm:", args.guide_norm)
            print("[CLI] pan_syn_mode:", args.pan_syn_mode)
            print("[CLI] use_residual:", args.use_residual)
            ...
            print(
                ...
                args.pan_hp_sigma,
                args.pan_hp_gain,
                args.pan_hp_gain_urban,
                args.pan_hp_gain_veg,
                args.pan_hp_gain_shadow,
            )
            ...
            print("[CLI] residual_guided:", args.residual_guided_radius, args.residual_guided_eps, args.residual_guided_mix)
            print("[CLI] ndvi:", args.ndvi_veg_lo, args.ndvi_veg_hi, args.nir_detail_boost, args.urban_detail_from_nir)
            print("[CLI] veg_luma:", args.veg_luma_lift, args.veg_luma_from_guide)
            print("[CLI] hotclip:", args.urban_hot_y, args.urban_hot_strength, args.final_softclip_margin)
            print("[CLI] radiometry:", args.radiometric_finish_mode, args.rad_p_lo, args.rad_p_hi, args.rad_soft_margin)
            print("[CLI] progress_enabled:", args.progress_enabled, "| progress_throttle_s:", args.progress_throttle_s)
